    ...
    while ( not_at_end ):
        ...
        if new_collision==True:
            ...
            for i in xrange(nkaons):
                ...
                pz = float(vals[3])
                q = int(vals[4])
                kaons.append([e,px,py,pz,q])

            # Read in the kaon info for this collision.
            protons = []
            line = infile.readline()
            nprotons = int(line)
            for i in xrange(nprotons):
                line = infile.readline()
                vals = line.split()
                e = float(vals[0])
                px = float(vals[1])
...
def draw_proton3D(origin=[(0,0,0)],pmom=[(1,1,1)]):
    ...
    lines = draw_line3D(origin=origin,pmom=pmom,color='yellow',lw=5)
